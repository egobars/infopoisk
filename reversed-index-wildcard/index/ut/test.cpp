...
TEST(IndexTest, TestDocument)
{
    ...
    {
        Finder finder(index, "horse", false);
...
TEST(IndexTest, TestDocumentWildcard)
{
    Index index(2, 3, 10);
    index.AddDocument("cat", "i have cat and dog and horse");
    index.AddDocument("dog", "i have dog and horse and crocodile");
    index.AddDocument("croco", "i have cat and dog and crocodile");
    {
        Finder finder(index, "cro*dile", true);
        std::vector<std::string> expected = { "dog", "croco" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
    {
        Finder finder(index, "c*dile", true);
        std::vector<std::string> expected = { "dog", "croco" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
    {
        Finder finder(index, "*codile", true);
        std::vector<std::string> expected = { "dog", "croco" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
    {
        Finder finder(index, "croco*", true);
        std::vector<std::string> expected = { "dog", "croco" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
    {
        Finder finder(index, "hor*", true);
        std::vector<std::string> expected = { "cat", "dog" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
    {
        Finder finder(index, "ca*", true);
        std::vector<std::string> expected = { "cat", "croco" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
    {
        Finder finder(index, "c*t", true);
        std::vector<std::string> expected = { "cat", "croco" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
    {
        Finder finder(index, "c*at", true);
        std::vector<std::string> expected = { "cat", "croco" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
    {
        Finder finder(index, "*", true);
        std::vector<std::string> expected = { "cat", "dog", "croco" };
        ASSERT_EQ(finder.GetDocuments(), expected);
    }
}
